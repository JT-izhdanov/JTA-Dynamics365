...
> **Repository scope: Dynamics 365 Sales only** — this is the working space for the Sales
> pack. The offering was originally designed around four Customer Engagement report packs
> plus a cross-app pack. Those are **deferred, not cancelled**: cross-app functionality is
> intended and will be built, but nothing deferred is documented, priced or positioned here
> until it exists. See
> [`docs/offering/roadmap.md`](docs/offering/roadmap.md#what-narrowing-costs-now-and-what-it-does-not)
> for what was removed, where to recover it, and what is deliberately kept general so a
> second pack stays cheap to add.
...
medallion architecture, and surfaced through a pre-built semantic model and report pack. The
implementable design for all of it —
including a **Fabric Data Agent** over the certified model — is
...
docs/
  ...
  architecture/   End-to-end solution design, reference architecture, medallion, conformance, RLS
